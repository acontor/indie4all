    ...
    public function busqueda(Request $request)
    {
        ...
        if ($request->has('q')) {
            ...
            $masters = DB::table('users')
                ...
                ->select('masters.id', 'masters.nombre', 'masters.imagen_logo')->where('masters.nombre', 'LIKE', "%$search%")->where('users.ban', 0)->take(5)->get();
            ...
            foreach ($masters as $master) {
                $temp = ['id' => $master->id, 'nombre' => $master->nombre, 'imagen_logo' => $master->imagen_logo, 'tipo' => 'Master'];
                array_push($data, $temp);
            }
        }
